...
<?php if ( true === $show_customiz_fields ) { ?>
	<p><input type="checkbox"
	...
	<p style="padding-top: 10px"><input type="checkbox"
		id="perfecty_push_send_featured_img"
		name="perfecty_push_send_featured_img"
		<?php
		if ( false !== $send_featured_img ) {
			echo 'checked="checked"';
		}
		?>
		/>
	<label for="perfecty_push_send_featured_img" id="perfecty_push_send_featured_img_label"><?php printf( esc_html__( 'Send featured image or first image found in content', 'perfecty-push-notifications' ) ); ?></label></p>
	<?php
} ?>

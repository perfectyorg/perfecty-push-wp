...
class PushServerTest extends WP_UnitTestCase {
  ...
  public function test_schedule_broadcast_async() {
    ...
    $next = wp_next_scheduled('perfecty_push_broadcast_notification_event', [$notification_id]);
  ...
  public function test_execute_broadcast_batch() {
    ...
    $next_after_first_execution = wp_next_scheduled('perfecty_push_broadcast_notification_event', [$notification_id]);

    // here we assume that the cron has kicked in and cleared the previous event
    wp_clear_scheduled_hook('perfecty_push_broadcast_notification_event', [$notification_id]); 

    $res = Perfecty_Push_Lib_Push_Server::execute_broadcast_batch($notification_id);
    // there should not be any scheduled events afterwards, $next_after_second_execution should BE false
    $next_after_second_execution = wp_next_scheduled('perfecty_push_broadcast_notification_event', [$notification_id]);
  ...
  public function test_execute_broadcast_batch_multiple() {
    ...
    $mocked_server
    ->shouldReceive([
      'flush' => [$mocked_server_result, $mocked_server_result] // first batch, 2 items
    ])
    ->once()
    ->shouldReceive([
  ...
  public function test_execute_broadcast_batch_notification_taken() {
    ...
    $next_after_execution = wp_next_scheduled('perfecty_push_broadcast_notification_event', [$notification_id]);
